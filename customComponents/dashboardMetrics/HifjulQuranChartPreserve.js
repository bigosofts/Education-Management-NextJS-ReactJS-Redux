...
const BarChartHifjulQuranPreserve = ({ getBarChartData }) => {
  ...
  useEffect(() => {
    ...
    function getData() {
      ...
      const mappedClasses =
        ...
        filteredClasses.map((item) => {
          return item.classID + ":" + item.batchNo;
        });
      ...
      if (savedData.length > 0) {
        ...
        filteredClasses.forEach((item) => {
          const key = `${item.classID}-${item.batchNo}`;
        ...
        savedData.forEach((item2, i) => {
          const classKey = `${item2.classID}-${item2.batch}`;
    ...
    function getLabel() {
      const filteredClasses =
        ...
        classes.filter((item) => {
          return (
            item.activeStatus === "active" && item.courseID === "hifjulquran"
          );
        });
  ...
  return (
    <>
      <h1 style={{ margin: "50px 0px" }}> Hifjul Quran Preserve Students: </h1>
...
export default BarChartHifjulQuranPreserve;
