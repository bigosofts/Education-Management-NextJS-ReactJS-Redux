            ...
            <div className='container mx-auto mt-10'>
                <div className='grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 grid-cols-1 gap-x-5 gap-y-20'>
                    {props.fromupdateform.data.map((item,i)=>(
                        <div hey={i} className="max-w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 relative">
                            
                            <a href="#">
                                <img className="rounded-t-lg w-full h-64" src={item.postImageLink} alt="" />
                            </a>
                            ...
                            <div className='absolute top-2 left-2'>
                                        ...
                                        Created at: {new Date(item.postCreatedDate).toLocaleDateString()}
                                        </div>
                                        ...
                                        <svg aria-hidden="true" className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd"></path></svg>
                                        Updated at: {new Date(item.postUpdatedDate).toLocaleDateString()}
                                        </div>
                            </div>
                            <div className="p-5">
                                <a href="#">
                                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.postTitle.en}</h5>
                                </a>
                                <a href="#">
                                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.postTitle.bn}</h5>
                                </a>

                                <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md"> Post User: {item.postUser}</p>
                                <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md"> Category: {item.postCategory.en}</p>
                                <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md"> ধরণ: {item.postCategory.bn}</p>
                                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md"> Post Id: {item.postId}</p>
                                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md"> Popularity: {item.postPopularity.en}</p>
                                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md"> জনপ্রিয়তা: {item.postPopularity.bn}</p>

                                <h5 className="mb-2 text-md font-bold tracking-tight text-gray-900 dark:text-white">Description</h5>
                                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 block max-h-24 overflow-y-scroll">{item.postDescription.en}</p>
                                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 block max-h-24 overflow-y-scroll">{item.postDescription.bn}</p>
