...
function updateMenuForm(props) {
    ...
    },[props.payload.menuTitle.en,props.payload.menuTitle.bn,props.payload.menuLink,props.payload.menuIcon,props.payload.menuType,props.payload.activeStatus]);
    ...
    const clickHandler =async (e)=>{
        ...
        const res = await updateData(menutitle,menutitlebn,menulink,menuicon,menutype,status,idValue);
    ...
    return (
        ...
        <div className="input-type">
            <input ref={menutitleref} onChange={onChangeHandler1} className="border w-full px-5 py-3 focus:outline-none" type='text' name='menutitle' placeholder='Enter menu title' value={menu.menutitle}></input>
        </div>
        <div className="input-type">
            <input ref={menutitlebnref} onChange={onChangeHandler2} className="border w-full px-5 py-3 focus:outline-none" type='text' name='menutitlebn' placeholder='বাংলায় মেনু টাইটেল লিখুন' value={menu.menutitlebn}></input>
        </div>
        <div className="input-type">
            <input ref={menulinkref} onChange={onChangeHandler3} className="border w-full px-5 py-3 focus:outline-none" type='text' name='menulink' placeholder='Enter menu link' value={menu.menulink}></input>
        </div>
        <div className="input-type">
            <input ref={menuiconref} onChange={onChangeHandler4} className="border w-full px-5 py-3 focus:outline-none" type='text' name='menuicon' placeholder='Enter menu icon' value={menu.menuicon}></input>
        </div>
        <div className="input-type">
            <input ref={menutyperef} onChange={onChangeHandler5} className="border w-full px-5 py-3 focus:outline-none" type='text' name='menutype' placeholder='Enter menu type' value={menu.menutype}></input>
        </div>
        <div className='flex gap-10 items-center'>
        {
                    props.payload.activeStatus == "active"
                        ...
                        <div className='form-check'>
                            <input ref={menuradio1ref} type="radio" value="Active" id="radioDefault1" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
...

                {
                    props.payload.activeStatus == "inactive"
                        ?
                        <div className='form-check'>
                            <input ref={menuradio2ref} type="radio" value="Inactive" id="radioDefault2" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
