...
function updateWidgetForm(props) {
    ...
    },[props.payload.widgetName,props.payload.widgetPayload,props.payload.activeStatus]);
    ...
    const clickHandler =async (e)=>{
        ...
        const status = widgetradio1 ? "active" : widgetradio2 ? "inactive" : "inactive";
        const idValue = props.data;
        

        const res = await updateData(widgettitle,widgetpayloadParsed,status,idValue);
    ...
    const onChangeHandler2 = (e) =>{
        setWidget({
            widgetPayload: JSON.parse(e.target.value)
        });
    }
    ...
    return (
        <form className='grid lg:grid-cols-3 w-full gap-5'>
            <div className="input-type">
                <input ref={widgettitleref} onChange={onChangeHandler1} value={widget.widgetName} className="border w-full px-5 py-3 focus:outline-none" type='text' name='widgettitle' placeholder='Enter widget title'></input>
            </div>
            <div>
                <textarea ref={widgetpayloadref} onChange={onChangeHandler2} value={JSON.stringify(widget.widgetPayload)} id="widgetpayload" name="widgetpayload" rows="1" className="border w-full px-5 py-3 focus:outline-none" placeholder='Enter Widget payload object here'></textarea>
            </div>
            ...
            <div className='flex gap-10 items-center'>
            {
                    props.payload.activeStatus == "active"
                        ...
                        <div className='form-check'>
                            <input ref={widgetradio1ref} type="radio" value="Active" id="radioDefault1" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
...

                {
                    props.payload.activeStatus == "inactive"
                        ?
                        <div className='form-check'>
                            <input ref={widgetradio2ref} type="radio" value="Inactive" id="radioDefault2" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
