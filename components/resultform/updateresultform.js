...
function updateResultForm(props) {
    ...
    const onChangeHandler6 = (e) =>{
        setResult({
            studentSubMark: JSON.parse(e.target.value)
        });
    }
    ...
    return (
        <form className='grid lg:grid-cols-3 w-full gap-5'>
            <div className="input-type">
                <input onChange={onChangeHandler3} ref={studentidref} className="border w-full px-5 py-3 focus:outline-none" type='text' name='studentuser' placeholder='Enter student User' value={result.studentUserId}></input>
            </div>
            <div className="input-type">
                <input onChange={onChangeHandler1} ref={resultrollnoref} className="border w-full px-5 py-3 focus:outline-none" type='number' name='resultrollno' placeholder='Enter result roll no' value={result.resultRollNo}></input>
            </div>
            <div className="input-type">
                <input onChange={onChangeHandler2} ref={regnumberref} className="border w-full px-5 py-3 focus:outline-none" type='number' name='resultregno' placeholder='Enter registration number' value={result.resultRegNo}></input>
            </div>
            <div className="input-type">
                <input onChange={onChangeHandler4} ref={madrashanameref} className="border w-full px-5 py-3 focus:outline-none" type='text' name='exammadrasha' placeholder='Enter Madrasha Name' value={result.studentExamMadrasha}></input>
            </div>
            <div className="input-type">
                <input onChange={onChangeHandler5} ref={examcentreref} className="border w-full px-5 py-3 focus:outline-none" type='text' name='examcentre' placeholder='Enter exam centre' value={result.studentExamCentre}></input>
            </div>
            
            <div className="input-type">
            <textarea onChange={onChangeHandler6} ref={markpayloadref} id="submark" name="submark" rows="1" className="border w-full px-5 py-3 focus:outline-none" placeholder='Enter Mark array of object' value={JSON.stringify(result.studentSubMark)}></textarea>
            </div>
            <div className="input-type">
                <input onChange={onChangeHandler7} ref={studentgraderef} className="border w-full px-5 py-3 focus:outline-none rounded-md" type='text' name='studentgrade' placeholder='Enter Student Grade Position' value={result.studentGrade}></input>
            </div>
            <div className="input-type">
            ...
            <div className='flex gap-10 items-center'>
            {
                    props.payload.activeStatus == "active"
                        ...
                        <div className='form-check'>
                            <input ref={resultradio1ref} type="radio" value="Active" id="radioDefault1" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
...

                {
                    props.payload.activeStatus == "inactive"
                        ?
                        <div className='form-check'>
                            <input ref={resultradio2ref} type="radio" value="Inactive" id="radioDefault2" name="status" className='form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer' checked/>
