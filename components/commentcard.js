        ...
        return (
            <div className='container mx-auto mt-10'>
                ...
                {props.fromupdateform.data.map((item,i)=>(
                    ...
                    <a href="#">
                        <img className="rounded-t-lg w-full h-64" src="/images/2.jpg" alt="" />
                    </a>
                    ...
                    <div className='absolute top-2 left-2'>
                                ...
                                Created at: {new Date(item.commentCreatedDate).toLocaleDateString()}
                                </div>
                                ...
                                <svg aria-hidden="true" className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd"></path></svg>
                                Updated at: {new Date(item.commentUpdatedDate).toLocaleDateString()}
                                </div>
                    </div>
                    <div className="p-5">
                        <a href="#">
                            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.userName.en} commented this</h5>
                        </a>
                        

                        <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md">Desgnation: {item.designation.en}</p>
                        <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md"> পদবী: {item.designation.bn}</p>
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md"> Username: {item.userName.en}</p>
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md">ইউজার নেমঃ {item.userName.bn} </p>
                        <p className="mb-3 font-normal text-orange-700 dark:text-orange-400 bg-slate-50 py-1 rounded-md relative">Comment icon: {item.commentIcon}<i style={{fontSize:24+"px"}} className={`${item.commentIcon} absolute right-1`}></i></p>
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 bg-slate-50 py-1 rounded-md">Comment ID: {item.commentId}</p>
                        
                        

                        <h5 className="mb-2 text-md font-bold tracking-tight text-gray-900 dark:text-white">Comment</h5>
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 block max-h-24 overflow-y-scroll">{item.comment.en}</p>
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400 block max-h-24 overflow-y-scroll">{item.comment.bn}</p>
            ...
            <div className='container mx-auto mt-10'>
                ...
                {comment.data.map((item,i)=>(
                    ...
                    <div className="p-5">
                        ...
                        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400  block max-h-24 overflow-y-scroll">{item.comment.bn}</p>
                        <div className='flex justify-end gap-2'>
                            <button onClick={()=>{updateHandler(item._id,comment); console.log(comment)}} className='w-2 px-5 py-5 allColorFont'><BiEdit size={24}/> </button>
